...
- A large (132px) copy of the tile's own Phosphor icon, "light" weight, oversized and bled off the top-right corner (`-right-[22px] -top-[26px]`) as a watermark. Both mask layers below sit inside one shared wrapper div that carries the position/size and a `.icon-watermark-spin` rotation (14s linear, around the vertical axis — a globe/coin turn, not a clock hand — see its `globals.css` comment) so the whole icon spins as one unit rather than the ring spinning around a static icon: a static faint fill (the accent color at ~15–20% alpha) for the icon's silhouette — using the icon path as a CSS `mask-image` (raw SVG path data, since a React `<Icon>` component can't be embedded in a mask) — and a stroke-outline mask (accent white on transparent, `stroke-width: 3` in the 256 viewBox) holding a second, independent spinning `conic-gradient` (`.icon-outline-spin`, 5s linear) at full-to-partial alpha of the same accent, so a brighter gradient ring also sweeps around the icon's outline as the whole thing turns. Filled (not stroked) for the base layer specifically because "light"-weight Phosphor paths are already thin bands; stroking that boundary traces both its inner and outer edge and doubles every line.
- This uses `{colors.purple}` and `{colors.pink}` as full card backgrounds for the first time (previously unused/only gradient-mesh accent stops respectively). With purple, pink, blue, and amber across the four cards, each now reads as a genuinely distinct hue at this pale `/25` tint — unlike the earlier citrine/amber and seafoam/primary pairings, which sat close enough in hue to read as similar colors.
