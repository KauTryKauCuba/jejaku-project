...
- **Skyline** (`{colors.seafoam}` — `#7fc0e0`): Gradient-mesh blob stop, `IconFlowBadge` flow-line stop, and one of Tax Records' gradient-watermark ring stops (previously also the Receipts Scanned card's own accent, moved to Pink).
- **Citrine** (`{colors.citrine}` — `#c9a227`): Defined as a token but not currently used in any shipped surface (previously the Total Spent gradient-watermark stat card's accent, moved to Purple) — reserve for a future surface.
- **Orange** (`{colors.orange}` — `#ea580c`): Defined as a token but not currently used in any shipped surface (briefly the Total Spent gradient-watermark stat card's accent, moved to Purple) — reserve for a future surface.
- **Purple** (`{colors.purple}` — `#9333ea`): The Total Spent gradient-watermark stat card's accent.
- **Pink** (`{colors.pink}` — `#db2777`): The Receipts Scanned gradient-watermark stat card's accent.
- **Green** (`{colors.green}` — `#16a34a`): Receipt Locations' gradient-watermark accent — the only true green in the palette (Skyline is blue-green).
...
**Gradient watermark cards** (`TotalSpentTile`, `ReceiptsScannedTile`, `TaxRecordsTile`, `WarrantyClaimsTile`, `LocationHistoryCard`) — a deliberate departure from the flat-bordered default (see Elevation & Depth), each keyed to its own accent, run at full saturation so every card reads as a clearly distinct color rather than a pale tint of a hue another card already owns: Total Spent → `{colors.purple}`, Receipts Scanned → `{colors.pink}`, Tax Records → `{colors.primary}`/`{colors.primary-subdued}`, Warranty Claims → `{colors.amber}`, Receipt Locations → `{colors.green}`. Not limited to the four dashboard stat tiles — Receipt Locations is a bigger content card (a draggable globe + list, not a single number), and gets the same treatment underneath its own content rather than instead of it. Structure, all built from `app/lib/iconMaskPaths.ts` + the `.icon-outline-spin`/`.icon-watermark-spin` keyframes in `globals.css`:
- Card background: `bg-gradient-to-br from-{accent}/25 via-canvas to-canvas` — a pale tint of the accent in the top-left corner, fading quickly to plain canvas. Every card shares this exact formula (only the accent and its opacity value vary slightly by design, e.g. Tax Records' `/25`-equivalent split across `primary-subdued`/`canvas` since it has no single accent token) so the set reads as one consistent pattern rather than a different treatment per card.
- A large (112.2px) copy of the card's own Phosphor icon, "light" weight, oversized and bled off the top-right corner (`-right-[18.7px] -top-[22.1px]`) as a watermark. Both mask layers below sit inside one shared wrapper div that carries the position/size and a `.icon-watermark-spin` rotation (14s linear, around the vertical axis — a globe/coin turn, not a clock hand — see its `globals.css` comment) so the whole icon spins as one unit rather than the ring spinning around a static icon: a static faint fill (the accent color at ~15–20% alpha) for the icon's silhouette — using the icon path as a CSS `mask-image` (raw SVG path data, since a React `<Icon>` component can't be embedded in a mask) — and a stroke-outline mask (accent white on transparent, `stroke-width: 3` in the 256 viewBox) holding a second, independent spinning `conic-gradient` (`.icon-outline-spin`, 5s linear) at full-to-partial alpha of the same accent, so a brighter gradient ring also sweeps around the icon's outline as the whole thing turns. Filled (not stroked) for the base layer specifically because "light"-weight Phosphor paths are already thin bands; stroking that boundary traces both its inner and outer edge and doubles every line. Each card also scatters 5 small copies of its own icon behind the main watermark (`WATERMARK_SATELLITES`), each spinning independently at its own speed/phase so they read as background texture, not a synchronized cluster.
- A card with real in-flow content on top of the watermark (Receipt Locations; also true of every stat tile's number/label) needs that content wrapped in its own `position: relative` element — the watermark divs are `position: absolute`, and CSS paints positioned (even z-index:auto) descendants after static in-flow ones as a group, regardless of DOM order between the two. Skipping this hid Receipt Locations' globe and list under the watermark the first time this was wired up; caught only by an actual screenshot, not by reading the diff.
- `{colors.purple}`, `{colors.pink}`, and `{colors.green}` are each used as a full card background for the first time here (previously unused, or only a gradient-mesh accent stop). With purple, pink, blue, amber, and green across the five cards, each now reads as a genuinely distinct hue at this pale `/25` tint — unlike the earlier citrine/amber and seafoam/primary pairings, which sat close enough in hue to read as similar colors.
- Every other stat/content tile in the app stays flat-bordered, shadow-free, no watermark — this pattern is scoped to these five cards, not a new default.
...
- Don't use `{colors.brand-dark-900}`, `{colors.citrine}`, or `{colors.orange}` yet — all are reserved tokens with no shipped surface; if you use one, document the new surface here.
...
- Same trap, different box: a gradient watermark card's `Select` (the this-month/3/6/12-month range dropdown) opens *below* the card, so its container can't carry `overflow-hidden` directly — that was clipping the open dropdown's lower options. The watermark itself still needs clipping to the rounded corners, so that `overflow-hidden` lives on its own `absolute inset-0` decorative div instead, a sibling of the actual card content rather than an ancestor of it. See any gradient watermark card for the pattern.
